 ...
 */
enum JValueType { JNull, JInt, JDouble, JBool, JString, JList, JDict };
...
protected:
  JObject parse_(std::string_view data, std::size_t data_size,
                 std::size_t &itor);
  void skipSpace(std::string_view data, std::size_t data_size,
                 std::size_t &itor, long long &error_line);
  std::pmr::string getString(std::string_view data, std::size_t data_size,
                             std::size_t &itor, long long error_line);
  JObject getNumber(std::string_view data, std::size_t data_size,
                    std::size_t &itor, long long error_line);
  JObject getBool(std::string_view data, std::size_t data_size,
                  std::size_t &itor, long long error_line);
  JObject getNull(std::string_view data, std::size_t data_size,
                  std::size_t &itor, long long error_line);
...
public:
   ...
   */
  std::string formatWrite(const JObject &jo, std::size_t indent = 4,
                          std::size_t n = 1);
};
} // namespace qjson
